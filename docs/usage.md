# immunespace/scintegrator: Usage
...
Here’s an example of a valid samplesheet:
...
```bash
nextflow run immunespace/scintegrator -profile docker -params-file params.yaml
```
...
```bash
nextflow pull immunespace/scintegrator
```
...
First, go to the [immunespace/scintegrator releases page](https://github.com/immunespace/scintegrator/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
